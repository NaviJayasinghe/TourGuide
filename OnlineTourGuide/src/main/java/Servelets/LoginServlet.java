		...
		if (palnner.Validate(UserName, password)){
			 	...
			 	//Create session for if user is tourguide
			 	 if (!TrvlPDetails.isEmpty()) {
			 		 	...
			 		 	String profile=TrvlPDetails.get(0).getProfilePic();
		                ...
		                System.out.println("Profile Pic Path: " + session.getAttribute("profile"));
		                
			 		 	LogedUserSession loggedInUser = new LogedUserSession(UserName, userID,profile);
